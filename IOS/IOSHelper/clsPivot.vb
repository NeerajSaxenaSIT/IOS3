...
Public Class Pivot
    ...
    Public Function UnPivotData(ByVal Aggregate As AggregateFunction, ByVal ParamArray ColumnFields As String()) As DataTable
        Dim dt As New DataTable()

        dt.Columns.Add("KPIName", GetType(String))
        dt.Columns.Add("KPIValues", GetType(Double))

        _SourceTable.Columns.Remove("Date")

        For Each kpiName As String In ColumnFields
            For Each dc As DataColumn In _SourceTable.Columns
                If dc.ColumnName.ToUpper = kpiName.ToUpper Then
                    Dim dr As DataRow = dt.NewRow()
                    dr("KPIName") = kpiName
                    dr("KPIValues") = GetData("", kpiName, Aggregate)
                    dt.Rows.Add(dr)
                End If
            Next
        Next
