...
namespace IOS.Configuration.TreeControl
{
    ...
    public partial class CustomTreeView : System.Windows.Forms.TreeView
    {
        ...
        public DragItemAddLevel DragItemAddLevel
        {
            get
            {
                return this._DragItemAddLevel;
            }
            set
            {
                this._DragItemAddLevel = value;
            }
        }
        /// <summary>
        ...
        private DragItemAddLevel _DragItemAddLevel;
        ...
        public CustomTreeView()
        {
            ...
            // Custom cursor handling
            if (this._DragItemAddLevel == 0)
            {
                this.DragItemAddLevel = DragItemAddLevel.Default;
            }
            else
            {
                this.DragItemAddLevel = this._DragItemAddLevel;
            }
            this._formDrag.Show();
        ...
        protected override void OnDragDrop(System.Windows.Forms.DragEventArgs e)
        {
            ...
            if (e.Data.GetDataPresent("System.Windows.Forms.TreeNode", false))
            {
                ...
                if (targetNode != dragNode)
                {
                    ...
                    ea.SourceNodeParent = dragNode.Parent;
                    ea.TargetNodeParent = targetNode.Parent;
                    TreeNode newTreeNode = (TreeNode)dragNode.Clone();
                    // Copy the node, add as a child to the destination node
                    if (this.DragItemAddLevel == DragItemAddLevel.Default && !targetNode.FullPath.StartsWith(dragNode.FullPath) && dragNode.Parent != targetNode)
                    {
                        IsValid = true;
                    ...
                    else if (this.DragItemAddLevel == DragItemAddLevel.Parallel &&
                        targetNode.Level == dragNode.Level)// && targetNode.Parent.Text== dragNode.Parent.Text
                    {
                        IsValid = true;
                        if (targetNode.Parent != null)
                        {
                            targetNode.Parent.Nodes.Insert(targetNode.Parent.Nodes.IndexOf(targetNode), newTreeNode);
                        }
                        else
                        {
                            targetNode.TreeView.Nodes.Insert(targetNode.TreeView.Nodes.IndexOf(targetNode), newTreeNode);
                        }
                        // targetNode.Expand();
                    }
                    ...
                    if (IsValid)
                    {
                        ...
                        ea.SourceNodeFullPath = newTreeNode.FullPath;
                        this.Cursor = Cursors.Default;
    ...
    #region DragItemAddingPosition enum
    [Serializable]
    public enum DragItemAddLevel
    {
        Default,
        Parallel
    }
    ...
    public class DragCompleteEventArgs : EventArgs
    {
        ...
        /// <summary>
        /// The Parent node that was being dragged
        /// </summary>
        public TreeNode SourceNodeParent
        {
            get
            {
                return this._sourceNodeParent;
            }
            set
            {
                this._sourceNodeParent = value;
            }
        }

        /// <summary>
        /// The Parent node that the source node was dragged onto.
        /// </summary>
        public TreeNode TargetNodeParent
        {
            get
            {
                return this._targetNodeParent;
            }
            set
            {
                this._targetNodeParent = value;
            }
        }

        public string SourceNodeFullPath
        {
            set
            {
                _soruceNodeFullPath = value;
            }
            get
            {
                return _soruceNodeFullPath;
            }
        }

        private TreeNode _targetNodeParent;
        private TreeNode _sourceNodeParent;

        private TreeNode _targetNode;
        private TreeNode _sourceNode;
        private string _soruceNodeFullPath;
    }
